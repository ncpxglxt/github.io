$(document).ready(function() {
    $(".h-pitch").on("click", function(e) {
        if (e.target && e.target.nodeName == "LI") {
            var liz = e.target.innerHTML;
            var kw = $(".search_kw").val();
            var cid = $(e.target).attr("cid");
            
            $(".h-pitch-on").html(liz);
//             $(".h-pitch-on").attr("data-url", $(e.target).attr("id"));
//             $(".h-pitch-on").attr("kid", $(e.target).attr("kid"));
            $('.h-pitch-on').data('enter',$(e.target).data('enter'));
            $(".h-pitch").hide();
            if (kw != "") {
                $(".search-click").trigger("click");
            }
            var cs = $(e.target).attr("id");
            $(".beijingtu ,.tuan,.shipinsucai,.pptmuban").hide();
            $("."+cs).show();
        }
    });
    $(".h-pitch-1").on({
    ...
    if (character !== undefined && character.length > 6) {
        $(".character").html(character.substring(0, 5) + "...");
    }
});

/**
 * placeholder.js  兼容ie8的表单提示
 */
...
$('.banner').owlCarousel({
  items:1,
    ...
    pagination: true,
    addClassActive: true,
    navigationText: ["<i class='iconfont icon-arrow-left'></i>", "<i class='iconfont icon-arrow-right'></i>"],
});
$('.shop-show-banner').owlCarousel({
  items:1,
    autoPlay: true,
    nav:false,
    navText:['',''],
